...
class PokerHand:
    ...
    hand_type: HandType
    primary_rank: Optional[Rank] = None  # For pair, three of a kind, four of a kind, straight
    ...
    def __str__(self) -> str:
        ...
        if self.hand_type == HandType.HIGH_CARD:
            ...
            rank_str = ",".join([rank_names[r] for r in sorted(self.ranks, reverse=True)])
...
class HandValidator:
    ...
    def hands_match(hand1: PokerHand, hand2: PokerHand) -> bool:
        ...
        if hand1.hand_type in [HandType.HIGH_CARD, HandType.PAIR, 
        ...
        if hand1.hand_type == HandType.TWO_PAIRS:
            return (hand1.primary_rank == hand2.primary_rank and 
                   hand1.secondary_rank == hand2.secondary_rank)
...
class HandParser:
    ...
    def _parse_suit(token: str) -> Suit:
        token = token.lower().rstrip('s')
        suit_map = {
            'heart': Suit.HEARTS, 'diamond': Suit.DIAMONDS,
            'club': Suit.CLUBS, 'spade': Suit.SPADES
        }
