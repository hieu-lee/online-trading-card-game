...
class DatabaseManager:
    ...
    async def save_game_result(self, game_id: str, winner: str, players: List[str], 
                             duration: int, started_at: datetime):
...
class UserSessionManager:
    ...
    async def authenticate_user(self, username: str, websocket) -> tuple[bool, str, Optional[User]]:
        """
        ...
        if self.is_username_online(username):
            return False, "This username exists and is online, choose another username", None
...
class HostManager:
    ...
    def ensure_host_exists(self) -> Optional[User]:
        ...
        if not self.current_host_id or not self.session_manager.get_user_by_id(self.current_host_id):
            return self.select_new_host()
...
async def authenticate_user(username: str, websocket) -> tuple[bool, str, Optional[User]]:
