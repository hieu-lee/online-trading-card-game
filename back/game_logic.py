...
class Round:
    ...
    def get_next_player_id(self, current_player_id: str) -> str:
        ...
        current_index = next((i for i, p in enumerate(active_players) 
                            if p.user.id == current_player_id), 0)
...
class Game:
    ...
    def remove_player(self, user_id: str):
        ...
        if user_id in self.players:
            ...
            if self.current_round:
                ...
                self.current_round.players = [p for p in self.current_round.players if p.user.id != user_id]
                # If it was this player's turn, move to the next player
                if self.current_round.current_player_id == user_id:
                    self.current_round.current_player_id = self.current_round.get_next_player_id(user_id)
    ...
    def start_new_round(self):
        ...
        if self.round_number == 1:
            ...
            if self.current_round:
                ...
                prev_index = next((i for i, p in enumerate(active_players) 
                                if p.user.id == prev_starter_id), 0)
    ...
    def call_bluff(self, user_id: str) -> Tuple[bool, str, Optional[str]]:
        ...
        return True, f"Bluff called! Hand {'exists' if hand_exists else 'does not exist'}", loser_id
    ...
    def restart_game(self):
        ...
        if self.waiting_players:
            from user_manager import user_session_manager  # local import to avoid circular dep
        ...
        self.waiting_players = [uid for uid in self.waiting_players if uid not in self.players]
    ...
    def get_game_state(self) -> Dict:
        ...
        return {
            ...
            "round_number": self.round_number,
            "current_player_id": self.current_round.current_player_id if self.current_round else None,
