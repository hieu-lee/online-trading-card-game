...
def translate_shorthand_to_spec(cmd: str) -> Optional[str]:
    ...
    if tokens[0] in {"2", "3", "4"}:
        ...
        elif len(tokens) == 4 and ((tokens[0] == "2" and tokens[2] == "3") or (tokens[0] == "3" and tokens[2] == "2")):
            # Full house shorthand
            pair_rank = _normalize_rank(tokens[1]) if tokens[0] == "2" else _normalize_rank(tokens[3])
            three_rank = _normalize_rank(tokens[3]) if tokens[2] == "3" else _normalize_rank(tokens[1])
    ...
    keywords = ['high card', 'pair', 'two pairs', 'three of a kind', 'straight', 'flush', 'full house', 'four of a kind', 'straight flush', 'royal flush']
...
class GameClient:
    ...
    def display_game_state(self):
        ...
        try:
            ...
            state_signature = str(self.game_state) + str(self.your_cards) + str(self.message_log[-5:])
        ...
        if online_users:
            console.print(f"Online users ({len(online_users)}): {', '.join(online_users)}")
            console.print()
        
        # Waiting players
        waiting_count = self.game_state.get("game_state", {}).get("waiting_players_count", 0)
        if waiting_count > 0:
    ...
    async def handle_server_message(self, message_str: str):
        ...
        try:
            ...
            if message.type == MessageType.GAME_STATE_UPDATE:
                ...
                self.add_message(message.data.get("message", "Please wait for next game"))
                self.your_cards = []
                self.display_game_state()
                ...
                self.add_message(f"Error: {message.data.get('message', 'Unknown error')}")
                self.display_game_state()
    ...
    async def process_command(self, command: str):
        ...
        if cmd == "start" and self.is_host:
            ...
            await self.send_message(MessageType.KICK_USER, {
    ...
    def show_help(self):
        ...
        console.print("[bold]Objective:[/bold] Be the last player standing by avoiding five losses.")
        console.print("Each round players receive cards equal to their losses + 1. On your turn you must either call a poker hand that you claim to have or call bluff on the previous player.")
        console.print()
        console.print("[bold]Poker Hand Rankings (low → high):[/bold] High Card, Pair, Two Pairs, Three of a Kind, Straight, Flush, Full House, Four of a Kind, Straight Flush, Royal Flush")
        console.print()
        ...
        console.print(" • [cyan]call <hand_spec>[/cyan] – Declare a poker hand. You can also just type the hand without the word 'call'. Examples: 'pair of jacks', '3 queens', '2 4 2 9', 'straight 10'.")
