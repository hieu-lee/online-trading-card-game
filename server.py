...
from user_manager import (
    initialize_user_system, authenticate_user, disconnect_user,
    get_online_users, get_host, is_host, get_all_connections
)
from game_logic import (
    add_player_to_game, remove_player_from_game, start_game, restart_game,
    make_hand_call, call_bluff, get_game_state, can_start_game, get_player_cards
)
...
class GameServer:
    ...
    async def handle_user_disconnect(self, user_id: str):
        ...
        if user:
            await self.broadcast_message(create_message(
    ...
    async def broadcast_game_state(self):
        ...
        state_message = create_message(MessageType.GAME_STATE_UPDATE, {
    ...
    async def _send_waiting_list_to_host(self):
        ...
        waiting_message = create_message(MessageType.PLAYER_UPDATE, {
    ...
    async def handle_user_join(self, websocket: WebSocketServerProtocol, data: Dict):
        ...
        if success and user:
            ...
            # Send join response
            response = create_message(MessageType.USER_JOIN, {
                "success": True,
                "message": message,
                "user_id": user.id,
            ...
            if user_cards:
                cards_data = [{"suit": card.suit.value, "rank": card.rank.value} for card in user_cards]
                cards_message = create_message(MessageType.PLAYER_UPDATE, {
            ...
            # Send error response
            error_response = create_message(MessageType.USERNAME_ERROR, {
                "success": False,
                "message": message
            })
        ...
        if can_start_game():
            if start_game():
                ...
                await self.broadcast_message(create_message(
                    ...
                    {"round_number": game_state["round_number"], "current_player_id": game_state.get("current_player_id")}
                ))
    ...
    async def handle_kick_user(self, host_id: str, target_username: str):
        ...
        if target_user:
            ...
            kick_message = create_message(MessageType.USER_KICKED, {
    ...
    async def handle_call_hand(self, user_id: str, data: Dict):
        ...
        if not spec:
            error_msg = create_message(MessageType.ERROR, {"message": "Hand specification is required"})
            await self.send_message_to_user(user_id, error_msg)
            return
        try:
            hand = HandParser.parse_hand_call(spec)
        except Exception as e:
            error_msg = create_message(MessageType.ERROR, {"message": f"Invalid hand specification: {e}"})
            await self.send_message_to_user(user_id, error_msg)
            return
        # Make the hand call
    ...
    async def handle_call_bluff(self, user_id: str):
        ...
        if success:
            bluff_message = create_message(MessageType.CALL_BLUFF, {
                "message": message,
                "loser_id": loser_id
            })
            ...
            if new_state.get("phase") == "playing":
                await self.broadcast_message(create_message(
                    MessageType.ROUND_START,
                    {"round_number": new_state.get("round_number"), "current_player_id": new_state.get("current_player_id")}
                ))
        else:
    ...
    async def handle_message(self, websocket: WebSocketServerProtocol, message_str: str):
        ...
        try:
            ...
            error_response = create_message(MessageType.ERROR, {
