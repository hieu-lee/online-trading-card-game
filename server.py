...
class GameServer:
    ...
    async def _process_request(self, path: str, request_headers: dict):
        """
        Return an HTTP 400 for any request that doesn't
        include an Upgrade: websocket header (e.g. HEAD / GET probes).
        """
    ...
    async def handle_call_bluff(self, user_id: str):
        ...
        if success:
            bluff_message = create_message(
                MessageType.CALL_BLUFF, {"message": message, "loser_id": loser_id}
            )
